...
class ReplayBuffer:
    ...
    def load_offline_dataset(self, data: Dict[str, np.ndarray]):
        ...
        self._actions[:n_transitions] = self._to_tensor(data["action"]) if len(data["action"].shape) == 2 else self._to_tensor(data["action"][..., None])
        ...
        self._mc_returns[:n_transitions] = self._to_tensor(data["mc_return"][..., None])
